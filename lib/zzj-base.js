 ...
 * @param {Function} callback 回调函数
 */
function remove$1(keys, callback) {
  if (Array.isArray(keys)) {
    keys.forEach(function (item) {
      remove(item.id, callback);
    });
  } else {
    remove(keys.id, callback);
  }
}
...
var ZWLApi = {
  ...
  receiveLogs: function receiveLogs(params) {
    ...
    return instance.post(baseUrl$1 + 'receiveLogs', params, { options: _extends$1({}, options, { log: false, loading: false, alert: false }) });
  }
};
  ...
  methods: {
          ...
          while (1) {
            switch (_context.prev = _context.next) {
              case 0:
                ...
                _context.next = 8;
                return ZWLApi.receiveLogs({ zzjWebLogsList: logs });
    ...
    startLoop: function startLoop() {
      ...
      CountDown.ticker({
        ...
        callback: function callback() {
          getAllByTime(function (res) {
            _this2.lock && Array.isArray(res) && res.length > 0 && _this2.uploadLogs(res);
          }, _this2.time, "h");
        }
      });
    },

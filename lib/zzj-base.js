...
function autoleave() {
  // 是否为局部的倒计时
  var isLocal = !!store$1.state.common.localExitTime;
  ...
  CountDown.ticker({
    ...
    callback: function callback() {
      if (isLocal) {
        setLocalExitTime(store$1.state.common.localExitTime - 1);
      } else {
...
var AutoLeave = {
  ...
  value: {
    inserted: function inserted(el, binding) {
      ...
      setLocalExitTime = function setLocalExitTime(p) {
        return store$1.commit('setLocalExitTime', p);
      };

...
var ZWLApi = {
  ...
  receiveLogs: function receiveLogs(params) {
    ...
    return instance.post(baseUrl$1 + 'receiveLogs', params, { options: _extends$1({}, options, { log: false }) });
  }
};
            ...
            switch (_context.prev = _context.next) {
              case 0:
                res.forEach(function (item) {
                  item.in_param = _JSON$stringify(item.in_param);
                  item.out_param = _JSON$stringify(item.out_param);
                });
                _context.next = 3;
                return ZWLApi.receiveLogs({ zzjWebLogsList: res });
